...
(def filter-sum-stable     fns/filter-sum-stable)
(def filter-sum-float4-stable fns/filter-sum-float4-stable)
(def filter-sum-numeric    fns/filter-sum-numeric)
(def filter-avg            fns/filter-avg)
(def filter-avg-stable     fns/filter-avg-stable)
(def filter-avg-numeric    fns/filter-avg-numeric)
(def filter-jsonb-agg      fns/filter-jsonb-agg)
(def filter-jsonb-agg-stable fns/filter-jsonb-agg-stable)
(def filter-jsonb-object-agg fns/filter-jsonb-object-agg)
(def filter-jsonb-object-agg-stable fns/filter-jsonb-object-agg-stable)
(def filter-json-object-agg  fns/filter-json-object-agg)
(def filter-json-object-agg-stable fns/filter-json-object-agg-stable)
...
(def filter-variance-samp-stable fns/filter-variance-samp-stable)
(def filter-stddev-samp    fns/filter-stddev-samp)
(def filter-stddev-samp-stable fns/filter-stddev-samp-stable)
(def filter-variance-pop   fns/filter-variance-pop)
(def filter-variance-pop-stable fns/filter-variance-pop-stable)
(def filter-stddev-pop     fns/filter-stddev-pop)
(def filter-stddev-pop-stable fns/filter-stddev-pop-stable)
...
(def filter-corr-stable    fns/filter-corr-stable)
(def filter-array-agg      fns/filter-array-agg)
(def filter-array-agg-stable fns/filter-array-agg-stable)
(def filter-string-agg     fns/filter-string-agg)
(def filter-string-agg-stable fns/filter-string-agg-stable)
...
(defn- parse-sql*
  ...
  (binding [params/*parse-db* db
            ...
            dq/*disable-planner* false
             ...
             ;; once per catalog table referenced. IdentityHashMap because
             ;; identity is what we want here — two structurally equal DB
             ;; snapshots are still separate parses — and because a
             ;; content-equality map would pay for equiv-db (a full datom
             ;; walk) on every lookup. Lifetime = this parse-sql call,
             ;; no leak.
             ;;
             ;; (This used to claim DB's Object.equals was broken and that
             ;; any WeakHashMap.get on a DB key threw. The throw was real
             ;; — defrecord's generated equals walked DB's custom seq
             ;; casting Datoms to Map.Entry — but it needed a hash-bucket
             ;; collision to surface, and it is fixed upstream as of
             ;; datahike 0.8.1770. Identity is the right choice on its own
             ;; merits regardless.)
