...
| **FULL JOIN with an aggregate answers one row per half** | `SELECT count(*) FROM ft FULL JOIN fu ON (ft.i = fu.k)` returns TWO rows, `5` and `4`; PostgreSQL returns `7`. The lowering runs two LEFT JOIN queries and concatenates their results, so an aggregate is computed per half and never over the join. Same for GROUP BY, and LIMIT applies per half | the combination has to happen before aggregation -- one query with two branches, rather than two queries and a set subtraction |
