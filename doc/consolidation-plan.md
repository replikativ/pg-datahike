...
| 0.7 | **shape.clj probes** | audit and review: FK names are made up (`fk_<hash of SQL>`); PK probe reads the table name with a regex and so misses `$1`; WHERE is ignored. **Scouted:** `pg_constraint` (conname/contype/conkey/condef), `pg_index` and `pg_attribute` are materialised, so the rows are there; the two blockers are `information_schema._pg_expandarray`, which nothing implements (the comment at catalog.clj:367 says "stubbed" but there is no stub), and `pg_index.indkey`, stored as the string `"[1]"` rather than an int2vector. | Implement `_pg_expandarray` as the SRF PostgreSQL defines it, store `indkey` as an array, serve all three probes from the catalog tables through the normal SQL path, delete shape.clj and the three handlers | pgjdbc metadata tests; oracle diff of the probe SQL |
| 0.8 | **LIMIT/OFFSET templating** | audit | Template only as an AST transform | fuzzer templated-vs-untemplated identity |
| 0.11 | **The fuzzer's join surface joins the gate** | **verified:** `bb fuzz join` reports 47 disagreements over 132 samples today — FULL JOIN and nullable-side conditions, each its own item above; INNER joins are clean | fix those, prune to zero, then add `:join` to the `all` surfaces in `differential.clj` | the surface itself |
...
| **FULL JOIN loses every right-only row** | `ft FULL JOIN fu ON (ft.j = fu.k AND ft.i = 10)` answers the 5 left rows and none of the 3 right-only ones; `count(*)` over a FULL JOIN is 5 where PostgreSQL says 8. Found by the new join fuzzer surface: 29 of its 35 FULL samples disagree | the FULL→two-LEFT rewrite keeps only the first half |
| **A condition over the nullable side leaks a datalog error** | `ft LEFT JOIN fu ON (ft.id = fu.id) WHERE fu.v IS NOT NULL` raises `Bad format for entity-id in pattern`, and `RIGHT JOIN … ON (… AND ft.i IS NOT NULL)` raises `Cannot resolve any more clauses` — both XX000. The WHERE emits `[(get-else $ ?fu_eid :fu/v …) ?v1]` OUTSIDE the or-join, reading the right entity var that the unmatched branch grounds to `:__null__`, and re-reads the column into a fresh variable instead of the one the join exposes | a clause outside the or-join must read the join's output variable, never the right entity var |
| **One run in six answered differently** | the same fuzzer corpus (seed 20260918, 132 samples) reported 123 disagreements once and 47 on every run before and after, including 29 failures in INNER joins that are otherwise clean. Not reproduced since; connection reuse, DDL churn, repeated prepared statements and fresh-vs-shared connections were each ruled out | unknown — worth a loop of the corpus when the join items above are fixed and the noise floor is zero |
| **A ref-based LEFT JOIN answers nothing** | over a `:db.type/ref` column: `person p LEFT JOIN company c ON p.company = c.db_id` returns no rows at all (INNER over the same columns is right). The lowering emits `[?c_eid :person/company ?c_eid]` — the RIGHT entity var on both sides, with the LEFT table's attribute — and leaves `[?c_eid :company/name ?c_name]` outside the or-join, where it filters. A second ON condition was additionally read by nobody | the ref branch needs the same treatment as the value branch: the LEFT entity var drives, every ON condition goes into the matched branch, right-side patterns move inside |
| **FULL JOIN fails over the extended protocol** | every FULL JOIN through pgjdbc raises `Received resultset tuples, but no field structure for them` (or an empty error when it has an ORDER BY); the same query over the simple protocol is right. One ON condition or several, it makes no difference | the FULL→two-LEFT rewrite (sql.clj:1496-1503) produces a statement the extended-protocol path describes differently than it executes |
