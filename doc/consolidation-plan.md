...
| 0.10 | **Cache and catalog-basis stability** *(one half fixed; the other has a deterministic repro now)* | **fixed:** a transaction holding an INSERT aborted with 40001 whenever any other session committed — the rebase refused to replay a buffer with inserts. 52 of 100 concurrent INSERT-then-UPDATE transactions on DISJOINT tables aborted; now 0, with no lost updates and real overlaps still detected. **verified, still open:** `ERROR: catalog changed while statement was being executed` fires for an UNRELATED DDL — session A opens a transaction and inserts, session B runs `CREATE TABLE other`, and A's next statement aborts. The basis captures the WHOLE schema plus every catalog row (`catalog/basis.clj capture*`), so any DDL anywhere invalidates every open statement's basis. **verified by reading:** the server-wide parse cache stores plans that captured the translating session's state atom (`session-value-expr!`) — mitigated by `params/session-dependent!` keeping those plans out of the shared cache; a two-session check of `pg_backend_pid()` now agrees. | compare only what the statement DEPENDS on (the admission certificate already models this for some shapes), or re-derive the basis where it is checked | the six validation surfaces green twice in a row; a loop of the reproduction stays clean |
