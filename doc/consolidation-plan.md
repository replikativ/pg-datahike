...
| 0.7 | **shape.clj probes** *(2 of 3 done: #200, #207)* | FK names were made up (`fk_<hash of SQL>`) — deleted, `pg_constraint` answers. The column-metadata probe hid a LEFT JOIN that multiplied one row per column into twenty; with that fixed the real query answers exactly as PostgreSQL does, so it is deleted too. The primary-key probe remains: it needs `information_schema._pg_expandarray` (nothing implements it), composite field selection `(x).n` (`RowGetExpression is not supported`), and `pg_index.indkey` as an array rather than the string `"[1]"`, plus `indnkeyatts` | implement those three, serve the probe's SQL from the catalog tables, delete the last probe and shape.clj with it | pgjdbc metadata tests; oracle diff of the probe SQL |
