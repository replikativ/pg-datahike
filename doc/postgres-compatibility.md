...
Exact sequence evaluation order remains open for COPY batches and for volatile
expressions produced by a non-trivial INSERT SELECT source. Those paths still
need the shared lazy source/candidate executor before pg-datahike can claim the
same cutoff point as PostgreSQL for every bulk write. This is tracked as the
remaining beta blocker `sequence-row-evaluation-order`.
