...
INSERT SELECT sources run at Execute against the statement snapshot rather than
being materialized while the statement is parsed. Stateful SELECT-list
expressions are represented without performing their effects while the source
is shaped. For each emitted row, pg-datahike resolves those expressions in
SELECT-list order, then fills target defaults in physical column order and
validates the candidate. A rejected candidate therefore consumes its own
sequence values but stops every later source row, and commit never repeats an
already materialized effect. When a volatile projection is also an `ORDER BY`
key, it is evaluated for every input row before sorting and `LIMIT`, matching
PostgreSQL's executor stage order.

This ordering guarantee currently covers stateful SELECT-list expressions.
Using `nextval` in predicates, grouping, aggregate arguments, HAVING, or window
definitions is rejected with `0A000` until those executor stages can carry the
same row-wise effect boundary; pg-datahike does not silently compare deferred
