...
(deftest identity-by-default-is-not-parsed-as-a-column-default
  (is (nil? (err (.execute *handler*
                           "CREATE TABLE identity_default_probe (id INTEGER GENERATED BY DEFAULT AS IDENTITY, value TEXT)"))))
  (is (nil? (err (.execute *handler*
                           "INSERT INTO identity_default_probe (value) VALUES ('first')"))))
  (is (= [["1" "first"]]
         (rows (.execute *handler*
                         "SELECT id, value FROM identity_default_probe"))))
  (is (= [["YES" "BY DEFAULT"]]
         (rows (.execute *handler*
                         (str "SELECT is_identity, identity_generation FROM information_schema.columns "
                              "WHERE table_name = 'identity_default_probe' AND column_name = 'id'")))))
  (is (nil? (err (.execute *handler*
                           "CREATE TABLE identity_always_probe (id INTEGER GENERATED ALWAYS AS IDENTITY, value TEXT)"))))
  (let [r (.execute *handler*
                    "INSERT INTO identity_always_probe VALUES (42, 'explicit')")]
    (is (= "428C9" (sqlstate r)))
    (is (re-find #"identity column.*id" (err r))))
  (is (nil? (err (.execute *handler* "BEGIN"))))
  (let [r (.execute *handler*
                    "INSERT INTO identity_always_probe VALUES (43, 'explicit-in-tx')")]
    (is (= "428C9" (sqlstate r)))
    (is (re-find #"identity column.*id" (err r))))
  (is (nil? (err (.execute *handler* "ROLLBACK"))))
  (is (nil? (err (.execute *handler*
                           "INSERT INTO identity_always_probe (value) VALUES ('implicit')"))))
  (is (= [["1" "implicit"]]
         (rows (.execute *handler*
                         "SELECT id, value FROM identity_always_probe"))))
  (is (= [["YES" "ALWAYS"]]
         (rows (.execute *handler*
                         (str "SELECT is_identity, identity_generation FROM information_schema.columns "
                              "WHERE table_name = 'identity_always_probe' AND column_name = 'id'")))))
  (is (= [["integer" "1" "2147483647"]]
         (rows (.execute *handler*
                         (str "SELECT data_type, min_value, max_value FROM pg_sequences "
                              "WHERE sequencename = 'identity_always_probe_id_seq'"))))))
